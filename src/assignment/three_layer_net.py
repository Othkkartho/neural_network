...
class ThreeLayerNet:
    ...
    def __init__(self, input_size, hidden_size1, hidden_size2, output_size, weight_init_std=0.01):
        ...
        self.params = {'W1': weight_init_std * np.random.randn(input_size, hidden_size1), 'b1': np.zeros(hidden_size1),
                       ...
                       'W3': weight_init_std * np.random.randn(hidden_size2, output_size), 'b3': np.zeros(output_size)}
    ...
    def gradient(self, x, t):  # 가중치 매개변수의 기울기를 구한다.
        ...
        grads = {'W1': self.layers['Affine1'].dW, 'b1': self.layers['Affine1'].db, 'W2': self.layers['Affine2'].dW,
                 'b2': self.layers['Affine2'].db, 'W3': self.layers['Affine3'].dW, 'b3': self.layers['Affine3'].db}
